...
export const BuyerPov = () => {
  ...
  const submitImage = async () => {
    if ((id as string) && id != undefined) {
      const ids = id as string;
      const result = await addImageToSupabase(imageFile, ids);
      console.log(result);
      toast({
        title: "Image Sent.",
  ...
  return (
    <div className="border-[10px] border-green-main w-[600px] h-[650px] rounded-3xl mt-4">
      ...
      <div className="container mx-auto w-11/12">
        <Text fontSize="lg" className="font-semibold mt-4">
          Proof of Transfer
        </Text>
        <hr></hr>
        <Text fontSize="md" className="italic mt-4">
          *Insert snap of transfer proof once transfer has been done
        </Text>
        ...
        <div className="flex justify-center mb-[10px]">
          <div>
            <button
              className="mb-5 bg-green-main w-24 h-10 font-semibold rounded-lg text-white"
              onClick={submitImage}
            >
              Submit
            </button>
